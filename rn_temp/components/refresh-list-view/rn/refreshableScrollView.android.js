...
import { ActivityIndicator, Animated, AsyncStorage, Dimensions, Easing, ScrollView, StyleSheet, Text, View } from 'react-native';
...
let RefreshableScrollView = class RefreshableScrollView extends ScrollView {
  ...
  renderRefreshHeader() {
    if (this.props.customRefreshView) {
      return <View style={[defaultHeaderStyles.header, this.props.refreshViewStyle]}>
          {this.props.customRefreshView(this.state.refreshStatus, this._offsetY)}
        </View>;
    }

    return <View style={[defaultHeaderStyles.header, this.props.refreshViewStyle, { height: this.props.refreshViewHeight }]}>
        <View style={defaultHeaderStyles.status}>
          {this.renderSpinner()}
          <Text style={defaultHeaderStyles.statusTitle}>{this.state.refreshTitle}</Text>
        </View>
        {this.props.displayDate && <Text style={[defaultHeaderStyles.date, this.props.dateStyle]}>{this.props.dateTitle + this.state.date}
        </Text>}
      </View>;
  }
  ...
  renderSpinner() {
    ...
    return <Animated.Image source={this.props.arrowImageSource} resizeMode="contain" style={[defaultHeaderStyles.arrow, this.props.arrowImageStyle, {
      transform: [{
  ...
  render() {
    return <ScrollView ref={c => this._scrollview = c} {...this.props} scrollEventThrottle={16} onScroll={this.onScroll} contentContainerStyle={{ minHeight: height }}
...
export { RefreshableScrollView as default };
